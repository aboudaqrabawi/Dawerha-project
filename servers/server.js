  ...
  db.query("SELECT * FROM users WHERE email = ?;", email, (err, result) => {
    ...
    res.send({ err: err });
     ...
     if (result.length > 0) {
      ...
      var mySql = `UPDATE items SET status = '${data.status}',acceptationStat = ${data.acceptationStat}, rejectionStat = ${data.rejectionStat} WHERE itemID = '${data.itemId}'`;
      db.query(mySql,(err, res) => {
               if (err)
                 console.log("error: ", err);
               })
        var mySql1 = `SELECT price,user_id FROM items WHERE itemID = '${data.itemId}' `;
        db.query(mySql1, (err, results) => {
          itemsInfo.price=results[0].price
          itemsInfo.userId=results[0].user_id
            console.log("results:",results);
            console.log("results[0]:",results[0]);
            console.log("itemsInfo::::::::::",itemsInfo);
        let mySql2 = `SELECT email FROM users WHERE userID = ${itemsInfo.userId} `;
        db.query(mySql2, (err, results) => {
            itemsInfo.receiver=results[0].email
            console.log("itemsInfo:",itemsInfo);
       
        console.log("itemsInfo:",itemsInfo);
        console.log(":::::::::::::::::::::",req.body.price,req.body.price, itemsInfo.receiver,data.itemId);
        var requestBody = {
          "sender_batch_header": {
